 ...
 * 修改备注：
 */
public class DeviceManagementAadapter extends BaseQuickAdapter<DeviceGetBean.DataBean.ListBean,BaseViewHolder>{
    public DeviceManagementAadapter(int layoutResId, @Nullable List<DeviceGetBean.DataBean.ListBean> data) {
        super(layoutResId, data);
        ScreenAdapterTools.getInstance().loadView(LayoutInflater.from(BaseApplication.getContext()).inflate(layoutResId,null));
    }

    @Override
    protected void convert(BaseViewHolder helper, DeviceGetBean.DataBean.ListBean item) {
